...
<body>
    ...
    <div id="hhh" class="container-fluid">
        <h1 id="novo">WEB STRANICA</h1>
        <section>
            <p id="tekst1">Ovdje možete komentirati kvalitetu naše web stranice. Možte nas obavijestiti o svim bugovima koje web stranica ima. Isto tako možte nam reći kako vam se sviđa dizajn i šta bi trebalo popraviti.</p>
        </section>      
    </div>
    <?php
        if(isset($_SESSION['Korisnicko_ime'])){
            echo"<form method='POST' action=".unesiKomentar($conn).">
                ...
                <input type='hidden' name='id_stranice' value='4'>
                <textarea id='komentar' name='Komentar' ></textarea><br>
                <button type='submit'name='komentarSubmit'>Komentiraj</button>
            </form>"; 
        }else{
